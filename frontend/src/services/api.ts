...
api.interceptors.request.use((config) => { const token = localStorage.getItem('policeops_token'); if (token) config.headers.Authorization = `Bearer ${token}`; return config; });
api.interceptors.response.use((response) => response, (error) => {
  if (error.response?.status === 401) {
    localStorage.removeItem('policeops_token');
    window.dispatchEvent(new CustomEvent('policeops:auth-expired'));
  }
