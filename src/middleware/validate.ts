...
export const uuidParams=z.object({id:z.string().uuid()}).passthrough();
export const assignmentParams=z.object({id:z.string().uuid(),recommendationId:z.string().uuid()});
...
export const coordinatesSchema=z.object({latitude:z.number().min(-90).max(90),longitude:z.number().min(-180).max(180),accuracy:z.number().nonnegative().max(10000).optional(),recorded_at:z.string().datetime().optional()}).strict();
...
export const fieldReportSchema=z.object({incident_type:z.enum(['COLLISION','CONGESTION','OBSTRUCTION','MEDICAL_EMERGENCY','OTHER']),severity:z.number().min(0).max(1),note:z.string().trim().min(3).max(2000),latitude:z.number().min(-90).max(90),longitude:z.number().min(-180).max(180),junction_id:z.string().uuid().nullable().optional(),client_request_id:z.string().uuid(),emergency_requested:z.boolean().default(false)}).strict();
export const deploymentPlanSchema=z.object({plan_date:z.string().date(),time_window:z.enum(['06:00-09:00','09:00-12:00','12:00-16:00','16:00-20:00','20:00-23:00'])}).strict();
export const planDecisionSchema=z.object({decision:z.enum(['APPROVED','MODIFIED','REJECTED']),notes:z.string().trim().max(1000).optional()}).strict();
